...
export function setupBlocks(element, cfg) {
    ...
    return Object.keys(cfg.blocks)
        .reduce((decoratedBlocks, block) => {
...
export function setLCPTrigger(lcp, blocks) {
    if (lcp) {
        if (lcp.complete) {
            postLCP(blocks, 'complete');
            return;
        }
        lcp.addEventListener('load', () => {
            postLCP(blocks, 'load');
        });
        lcp.addEventListener('error', () => {
            postLCP(blocks, 'error');
        });
        return;
    }
    postLCP(blocks, 'none');
